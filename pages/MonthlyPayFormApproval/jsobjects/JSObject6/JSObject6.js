export default {
  ...
  async generatePDF() {
    ...
    try {
      doc.addImage(base64Logo, "JPEG", 10, 5, 20, 30);
    } catch (e) {
    ...
    const approvedBy = (appsmith && appsmith.store && appsmith.store.user && appsmith.store.user.username) ? appsmith.store.user.username : "";
    const approvedAt = new Date().toLocaleString();
		
		const createdBy = TableCurrentM.tableData[0].createdby ?? "";
		const createdAt = TableCurrentM.tableData[0].createdat.toLocaleString() ?? "";
		
		const reviewedBy = TableCurrentM.tableData[0].reviewedby ?? "";
		const reviewedAt = TableCurrentM.tableData[0].reviewedat.toLocaleString() ?? "";

    doc.setFont("helvetica", "italic");
    doc.setFontSize(10);

    doc.text(`Reviewed by: ${reviewedBy}`, 115, footerY);
    doc.text(`Date: ${reviewedAt}`, 115, footerY + 6);
		
		doc.text(`Created by: ${createdBy}`, 15, footerY);
    doc.text(`Date: ${createdAt}`, 15, footerY + 6);
		
		doc.text(`Approved by: ${approvedBy}`, 210, footerY);
    doc.text(`Date: ${approvedAt}`, 210, footerY + 6);
